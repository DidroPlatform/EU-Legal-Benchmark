...
class TestCanonicalLoader(unittest.TestCase):
    ...
    def test_loader_uses_explicit_messages_when_present(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "messages.jsonl"
            path.write_text(
                (
                    '{"schema_version":"legal_eval_v1","id":"m1","dataset":"d","task_type":"reference_qa",'
                    '"prompt":"P","reference_answers":["A"],'
            ...
            ds = DatasetConfig(name="messages", path=str(path))
            examples = load_examples(ds)
            self.assertEqual(len(examples), 1)
            self.assertEqual(len(examples[0].messages), 3)
            self.assertEqual(examples[0].messages[0].role, "user")
            self.assertEqual(examples[0].messages[0].content, "Q1")
