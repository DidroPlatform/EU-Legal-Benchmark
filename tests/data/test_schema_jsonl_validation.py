...
class TestSchemaJsonlValidation(unittest.TestCase):
    ...
    def test_invalid_messages_shape_is_reported(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            path = Path(td) / "messages-invalid.jsonl"
            path.write_text(
                (
                    '{"schema_version":"legal_eval_v1","id":"bad-msg","dataset":"d","task_type":"reference_qa",'
            ...
            self.assertEqual(report["invalid_rows"], 1)
            self.assertIn("messages[0].role", " ".join(report["errors"][0]["errors"]))
