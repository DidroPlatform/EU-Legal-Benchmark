...
class TestRunInterrupt(unittest.TestCase):
    ...
    def test_judging_interrupt_writes_partial_outputs(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            config = self._build_config(root)
            config.run.response_parallel_workers = 1
            config.run.judge_parallel_workers = 1
            output = StringIO()
            call_count = {"judge": 0}
            ...
            self.assertEqual(summary.get("run_status"), "interrupted")
            self.assertEqual(summary.get("interrupted_stage"), "judging")
            ...
            text = output.getvalue()
            self.assertIn("stage=judge_phase_interrupted", text)
